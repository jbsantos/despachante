...
def create_app(config_name):
    ...
    def missao():
        ...
        #return redirect('/admin/missao/edit/?id=7')
    ...
    @app.route('/missaoconcluida/', methods=['GET'])
    @app.route('/missaoconcluida/<limit>', methods=['GET'])
    @auth_token_required
    def get_missao_concluido():
        header = {
            'access_token': request.headers['access_token'],
            "token_type": "JWT"
        }

        missaoconcluida = MissaoConcluidaController()
        response = missaoconcluida.get_missao_concluida()
        return Response(json.dumps(response, ensure_ascii=False), mimetype='application/json'), response['status'], header
